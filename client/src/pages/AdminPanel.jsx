...
export default function AdminPanel() {
  ...
  const loadFooterData = useCallback(async () => {
    try {
      ...
      setFooterSettings(settingsData.settings);
  ...
  useEffect(() => {
    if (section === 'footer') loadFooterData();
  }, [section, loadFooterData]);

  useEffect(() => {
    if (section === 'services') loadServices();
  }, [section, loadServices]);
  ...
  async function saveFooterSettings() {
    ...
    try {
      await api('/api/footer/settings', {
        method: 'PUT',
        body: JSON.stringify(footerSettings),
      });
      toast.push('Footer settings saved successfully!', 'success');
    } catch (error) {
      toast.push('Failed to save footer settings', 'error');
    }
  }
  ...
  async function saveFooterLink(e) {
    ...
    try {
      if (editingFooterLink) {
        await api(`/api/footer/links/${editingFooterLink._id}`, {
          method: 'PUT',
          body: JSON.stringify(footerLinkForm),
        });
        toast.push('Footer link updated successfully!', 'success');
      } else {
        await api('/api/footer/links', {
          method: 'POST',
          body: JSON.stringify(footerLinkForm),
        });
        toast.push('Footer link created successfully!', 'success');
      }
      ...
      toast.push('Failed to save footer link', 'error');
    }
  }
  ...
  async function deleteFooterLink(id) {
    try {
      ...
      toast.push('Footer link deleted successfully!', 'success');
      await loadFooterData();
    } catch (error) {
      toast.push('Failed to delete footer link', 'error');
    }
  }

  function editFooterLink(link) {
  ...
  async function loadServices() {
    try {
      const data = await api('/api/services/admin/all');
      setServices(data);
    } catch (error) {
  ...
  async function saveService(e) {
    ...
    try {
      if (editingService) {
        await api(`/api/services/admin/${editingService._id}`, {
          method: 'PUT',
          body: JSON.stringify(serviceForm),
        });
        toast.push('Service updated successfully!', 'success');
      } else {
        await api('/api/services/admin', {
          method: 'POST',
          body: JSON.stringify(serviceForm),
        });
        toast.push('Service created successfully!', 'success');
      }
      ...
      toast.push('Failed to save service', 'error');
    }
  }
  ...
  async function deleteService(id) {
    try {
      ...
      toast.push('Service deleted successfully!', 'success');
      await loadServices();
    } catch (error) {
      toast.push('Failed to delete service', 'error');
    }
  }
  ...
  return (
    <div className={`admin-layout ${sidebarCollapsed ? 'admin-layout--collapsed' : ''}`}>
      ...
      <aside className={`admin-sidebar ${sidebarOpen ? 'admin-sidebar--open' : ''}`}>
        ...
        <button
          type="button"
          className={`sidebar-link ${section === 'services' ? 'active' : ''}`}
        ...
        <button
          type="button"
          className={`sidebar-link ${section === 'footer' ? 'active' : ''}`}
          onClick={() => goSection('footer')}
        >
          <span className="si">🦶</span>
          <span className="sidebar-label">Footer</span>
        </button>
        <button
          type="button"
      ...
      <div className="admin-content">
        ...
        {section === 'services' ? (
          <div className="admin-section active" id="admin-services">
            <div className="admin-header">
              ...
              <form onSubmit={saveService}>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    placeholder="Brief description for navbar and cards"
                    maxLength={200}
                  />
                </div>
                ...
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    placeholder="Full service content (supports markdown-like formatting)"
                    style={{ minHeight: 200 }}
                    required
                  />
                </div>
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      placeholder="e.g., Starting from ₹15,000/month"
                    />
                  </div>
                </div>
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setServiceForm({ ...serviceForm, orderIndex: parseInt(e.target.value) || 0 })}
                      placeholder="0"
                      min="0"
                    />
                  </div>
                ...
                <div className="settings-section">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      type="text"
                      value={serviceForm.metaKeywords}
                      onChange={(e) => setServiceForm({ ...serviceForm, metaKeywords: e.target.value })}
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setServiceForm({ ...serviceForm, ogImage: e.target.value })}
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  {editingService && (
                    <button type="button" className="cancel-btn" onClick={cancelServiceEdit}>
                      Cancel
                    </button>
                  )}
                </div>
                ...
                <tbody>
                  {services.map((service) => (
                    <tr key={service._id}>
                      ...
                      <td>
                        <span className={`status-badge ${service.isActive ? 'status-new' : 'status-contacted'}`}>
                          {service.isActive ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td>
                        <span className={`status-badge ${service.featured ? 'status-new' : 'status-contacted'}`}>
        ...
        {section === 'footer' && footerSettings ? (
          <div className="admin-section active" id="admin-footer">
            <div className="admin-header">
            ...
            <div className="settings-section">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setFooterSettings({ ...footerSettings, logo_url: e.target.value })}
              ...
              <button type="button" className="save-btn" onClick={saveFooterSettings}>
            ...
            <div className="settings-section">
              <h3>Footer Links Manager</h3>
              
              {/* Add/Edit Link Form */}
              <div className="admin-form" style={{ marginBottom: '2rem' }}>
                <h4>{editingFooterLink ? 'Edit Footer Link' : 'Add New Footer Link'}</h4>
                <form onSubmit={saveFooterLink}>
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      <input
                        ...
                        placeholder="e.g., Services, Company, Quick Links"
                        required
                      />
                    </div>
                    <div className="form-group">
                  ...
                  <div className="form-row">
                    ...
                    <div className="form-group">
                      ...
                      <input
                        type="number"
                        value={footerLinkForm.order_index}
                        onChange={(e) => setFooterLinkForm({ ...footerLinkForm, order_index: parseInt(e.target.value) || 0 })}
                        placeholder="0"
                        min="0"
                      />
                    </div>
                  ...
                  <div className="form-group">
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <input
                        type="checkbox"
                        checked={footerLinkForm.is_active}
                        onChange={(e) => setFooterLinkForm({ ...footerLinkForm, is_active: e.target.checked })}
                      />
                      Active
                    </label>
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    {editingFooterLink && (
                      <button type="button" className="cancel-btn" onClick={cancelFooterLinkEdit}>
                        Cancel
                      </button>
                    )}
                  </div>
              ...
              <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden' }}>
                  ...
                  <tbody>
                    {footerLinks.map((link) => (
                      <tr key={link._id}>
                        ...
                        <td>
                          <span className={`status-badge ${link.is_active ? 'status-new' : 'status-contacted'}`}>
                        ...
                        <td>
                          ...
                          <button
                            ...
                            onClick={() => setConfirmDel({
                              ...
                              message: 'This cannot be undone.',
                              onConfirm: async () => {
                                await deleteFooterLink(link._id);
                                setConfirmDel(null);
                              },
                              onCancel: () => setConfirmDel(null)
                            })}
                ...
                {footerLinks.length === 0 && (
                  <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text3)' }}>
                    No footer links added yet. Add your first link above.
                  </div>
                )}
              </div>
            </div>
